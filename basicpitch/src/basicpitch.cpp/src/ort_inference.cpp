                                                        ...
                                                        int length)
{
    ...
    // Debian's libonnxruntime links system libonnx: at first session creation
    // both register the same op schemas, printing spurious
    // "Schema error: ... already registered" warnings to stderr.
    // Suppress them unless verbose.
    int saved_stderr = -1;
    int devnull = -1;
